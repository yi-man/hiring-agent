...
import { fetchJobDescriptionRegenerateRunWithEvents } from '@/lib/jd/client';
import {
...
import type {
  ...
  JobDescriptionRegenerateRunStage,
  JobDescriptionRegenerateRunStatus,
} from '@/lib/jd/regenerate-run-repo';

const terminalStatuses: JobDescriptionRegenerateRunStatus[] = ['success', 'failed'];
...
const stageLabels: Record<JobDescriptionRegenerateRunStage, string> = {
  queued: '任务创建',
...
const stageOrder: JobDescriptionRegenerateRunStage[] = [
  'queued',
  'input_preparation',
  'llm_generation',
  'saving',
...
function formatDuration(run: JobDescriptionRegenerateRunDto) {
...
function eventToneClass(level: JobDescriptionRegenerateRunEventDto['level']) {
        ...
        <div className="grid gap-1">
          {value.map((item, index) =>
            isPlainObject(item) ? (
              ...
              >
                <span className="min-w-0 truncate">{String(item.label ?? item.id ?? index)}</span>
...
export function JDRegenerateRunExecution({
  jobDescriptionId,
  runId,
}: {
  ...
  const [run, setRun] = useState<JobDescriptionRegenerateRunDto | null>(null);
  const [events, setEvents] = useState<JobDescriptionRegenerateRunEventDto[]>([]);
  ...
  const loadRun = useCallback(
    async (options?: { silent?: boolean }) => {
      ...
      setError('');
      try {
        const data = await fetchJobDescriptionRegenerateRunWithEvents(jobDescriptionId, runId);
        setRun(data.run);
        setEvents(data.events);
      } catch (e) {
        setError(e instanceof Error ? e.message : '加载 JD 重新生成进度失败');
      } finally {
        setIsLoading(false);
  ...
  const executionPath = currentPathWithSearch(
    `/jd-generator/${jobDescriptionId}/regenerate-runs/${runId}`,
    searchParams,
  );
  const detailHref = withReturnTarget(`/jd-generator/${jobDescriptionId}`, {
    href: executionPath,
    label: '返回执行页',
  });
  const contextHref = withReturnTarget(`/jd-generator/${jobDescriptionId}/context`, {
    href: executionPath,
    label: '返回执行页',
  });
  ...
  if (isLoading) {
    return (
      <div className="text-muted-foreground py-12 text-center text-sm">正在加载重新生成执行页…</div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="border-border flex flex-col gap-3 border-b pb-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            {run.extraInstruction ? `追加要求：${run.extraInstruction}` : '无追加要求'} · {run.tone}{' '}
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button as={Link} className="gap-2" href={contextHref} variant="bordered">
      ...
      {run.status === 'success' ? (
        <div className="rounded-md border border-emerald-200 bg-emerald-50/70 px-4 py-3 text-sm text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-200">
          JD 已重新生成完成，可以进入详情页查看更新后的内容。
          {contextHref ? (
            <Link className="ml-2 font-medium underline" href={contextHref}>
              查看本次上下文
            </Link>
          ) : null}
        </div>
      ...
      {run.status === 'failed' ? (
        <div className="border-destructive/30 bg-destructive/10 text-destructive flex items-start gap-2 rounded-md border px-4 py-3 text-sm">
          ...
          <span>{run.errorMessage || 'JD 重新生成失败'}</span>
        </div>
      ) : null}
      ...
      <section className="grid gap-3 md:grid-cols-4">
        ...
        <div className="border-border rounded-lg border p-4">
          <div className="text-muted-foreground text-xs">语气</div>
          <div className="text-foreground mt-2 truncate text-base font-semibold">{run.tone}</div>
        </div>
      </section>
