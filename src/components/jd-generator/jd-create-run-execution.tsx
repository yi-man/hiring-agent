...
import { fetchJobDescriptionCreateRunWithEvents } from '@/lib/jd/client';
import {
...
import type {
  ...
  JobDescriptionCreateRunStage,
  JobDescriptionCreateRunStatus,
} from '@/lib/jd/create-run-repo';

const terminalStatuses: JobDescriptionCreateRunStatus[] = ['success', 'failed'];
...
const stageLabels: Record<JobDescriptionCreateRunStage, string> = {
  queued: '任务创建',
  input_preparation: '整理岗位输入',
  llm_generation: '生成 JD 内容',
  saving: '保存到工作台',
  completed: '完成',
};

const stageOrder: JobDescriptionCreateRunStage[] = [
  'queued',
  'input_preparation',
  'llm_generation',
  'saving',
...
function formatDuration(run: JobDescriptionCreateRunDto) {
...
function eventToneClass(level: JobDescriptionCreateRunEventDto['level']) {
...
export function JDCreateRunExecution({ runId }: { runId: string }) {
  const searchParams = useSearchParams();
  const returnTarget = getReturnTarget(searchParams, {
    href: '/jd-generator',
    label: '返回列表',
  });
  const [run, setRun] = useState<JobDescriptionCreateRunDto | null>(null);
  const [events, setEvents] = useState<JobDescriptionCreateRunEventDto[]>([]);
  ...
  const loadRun = useCallback(
    async (options?: { silent?: boolean }) => {
      ...
      setError('');
      try {
        const data = await fetchJobDescriptionCreateRunWithEvents(runId);
        setRun(data.run);
        setEvents(data.events);
      } catch (e) {
        setError(e instanceof Error ? e.message : '加载 JD 创建进度失败');
      } finally {
        setIsLoading(false);
  ...
  const detailHref = run?.jobDescriptionId
    ? withReturnTarget(`/jd-generator/${run.jobDescriptionId}`, {
        href: currentPathWithSearch(`/jd-generator/create-runs/${runId}`, searchParams),
        label: '返回执行页',
      })
    : null;
  const contextHref = run?.jobDescriptionId
    ? withReturnTarget(`/jd-generator/${run.jobDescriptionId}/context`, {
        href: currentPathWithSearch(`/jd-generator/create-runs/${runId}`, searchParams),
        label: '返回执行页',
      })
    : null;
    ...
    <div className="space-y-4">
      <div className="border-border flex flex-col gap-3 border-b pb-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <h1 className="text-foreground text-2xl font-semibold tracking-normal">JD 创建执行</h1>
            <Chip className={`border text-xs ${meta.className}`} size="sm" variant="flat">
              {meta.label}
            </Chip>
          </div>
          <p className="text-muted-foreground mt-1 text-sm">
            {run.department} · {run.position} · {formatTime(run.updatedAt)}
          </p>
        </div>
      ...
      {run.status === 'failed' ? (
        <div className="border-destructive/30 bg-destructive/10 text-destructive flex items-start gap-2 rounded-md border px-4 py-3 text-sm">
          ...
          <span>{run.errorMessage || 'JD 创建失败'}</span>
        </div>
      ) : null}
      ...
      <section className="grid gap-3 md:grid-cols-4">
        ...
        <div className="border-border rounded-lg border p-4">
          <div className="text-muted-foreground text-xs">工作地点</div>
          <div className="text-foreground mt-2 truncate text-base font-semibold">
            {run.workLocations.join('、')}
          </div>
        </div>
      </section>
