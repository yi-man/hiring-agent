...
export default async function JDCandidateInterviewPage({
  params,
}: {
    ...
    <section className="container mx-auto px-4 py-8">
      {!session?.user ? (
        <div className="border-border bg-background/60 rounded-xl border p-8 text-center backdrop-blur">
          ...
          <p className="text-muted-foreground mt-2 text-sm">登录本地账号后即可查看面试详情。</p>
          <div className="mt-6 flex justify-center">
            <SignInButton />
          </div>
        </div>
      ) : (
        <CandidateInterviewDetail candidateId={candidateId} jobDescriptionId={id} />
      )}
    </section>
  );
}
