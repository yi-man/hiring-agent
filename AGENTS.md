...
- 使用 **pnpm** 管理依赖；Node ≥ 20、pnpm ≥ 10；路径别名 **`@/`** 指向 `src/`。
- **TypeScript** 严格类型，避免滥用 `any`；新增逻辑配套测试与类型；遵循 ESLint + Prettier（提交前 lint-staged）。
- **提交**：commitlint 约定式提交（`feat` / `fix` / `docs` / `style` / `refactor` / `test` / `chore`）；pre-commit 会跑 lint-staged、`pnpm type-check`、`pnpm test:ci`。
...
- `pnpm test:e2e:playwright:workflow`：**Workflow Learning** 真实链路 E2E（需 **PostgreSQL** 会话种子、`OPENAI_API_KEY`、本机已 `playwright install`；与 chat 文档流用例同为 **无 mock**）。根目录 `.npmrc` 对 `playwright` 做了 **public-hoist**，避免 pnpm 双份包导致 Playwright 报 `test.describe()` 上下文错误。
- `pnpm test:integration:chat` / `pnpm test:integration:auth`：真实 PostgreSQL/Redis 等依赖的集成测试。
...
- `pnpm clean` / `pnpm reinstall`：清理与重装。
